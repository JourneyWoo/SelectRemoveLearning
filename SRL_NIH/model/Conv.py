...
# -*- coding: utf-8 -*-
"""
Created on Sat Oct 28 10:32:05 2017
...
def cnn_model():
    ...
    print '@Gender  The number of training steps: ', c_step
    
    #data set
    hc_train = '/Users/wuzhenglin/Python_nice/SAL_BRAIN/brain.tfrecords'
    ...
    hc_test = '/Users/wuzhenglin/Python_nice/SAL_BRAIN/brain_test.tfrecords'
    ...
    c_train = '/Users/wuzhenglin/Python_nice/SAL_BRAIN/brain_gender.tfrecords' 
    ...
    c_test = '/Users/wuzhenglin/Python_nice/SAL_BRAIN/brain_gender_test.tfrecords'
    ...
    hc_train_loss = np.empty(hc_step - 1)
    hc_train_acc = np.empty(hc_step - 1)
    
    c_train_loss = np.empty(c_step - 1)
    c_train_acc = np.empty(c_step - 1)
    
    ind_dis = np.empty(c_step)
    
    #matrix for plot weight of last layer
    f, hch_plots = plt.subplots(int((hc_step + c_step) / 16), 16, figsize=(100, 100))
    ...
    for chc in range(3):
        ...
        #First layer
        W_conv1 = weight_variable("w1.bin", chc_select, [5, 5, 1, 4])
        ...
        #Second layer
        W_conv2 = weight_variable("w2.bin", chc_select, [5, 5, 4, 8])
        ...
        #Full connect layer
        W_fc1 = weight_variable("w3.bin", chc_select, [32 * 32 * 8, hidden])
                ...
                for ep in range(epochs):
                    ...
                    for i in range(num//batch_size):
                        ...
                        if step == stepnum and chc_select == hc:
                            ...
                            w1, b1, w2, b2, w3, b3, w4, b4 = sess.run([W_conv1, b_conv1, W_conv2, b_conv2, W_fc1, b_fc1, W_fc2, b_fc2], feed_dict = feed_dict)
                            w1.tofile("w1.bin")
                            b1.tofile("b1.bin")
                            w2.tofile("w2.bin")
                            b2.tofile("b2.bin")
                        ...
                        if chc_select == hc:
                            ...
                            hch_plots[int((step - 1) / 16), int(((step - 1) % 16))].axis('off')
                            hch_plots[int((step - 1) / 16), int(((step - 1) % 16))].imshow(weight_4_delta, cmap = plt.cm.YlGn)
                        
                        if chc_select == c:
                            
                            print "@Gender  step: %d  loss: %.9f  accuracy: %.3f" % (step, los, acc)
                            
                            if step < 49:
    
                                hch_plots[int(int(((step + hc_step) - 1) / 16)), ((step + hc_step - 1) % 16)].axis('off')
                                hch_plots[int(int(((step + hc_step) - 1) / 16)), ((step + hc_step - 1) % 16)].imshow(weight_4_delta, cmap=plt.cm.Blues)
