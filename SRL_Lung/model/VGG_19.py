...
def cnn_model():
    ...
    #data set
    hc_train = '/media/stargazer/Hurt Cloud/dataset/body/tfrecords/lungdata/hc_train.tfrecords'
    ...
    hc_test = '/media/stargazer/Hurt Cloud/dataset/body/tfrecords/lungdata/hc_test.tfrecords'
    ...
    c_train = '/media/stargazer/Hurt Cloud/dataset/body/tfrecords/lungdata/c_train.tfrecords' 
    ...
    c_test = '/media/stargazer/Hurt Cloud/dataset/body/tfrecords/lungdata/c_test.tfrecords'
    ...
    for chc in range(3):
        ...
        #1.1
        W_conv1 = weight_variable("w1.bin", chc_select, [3, 3, 1, 8])
        b_conv1 = bias_variable("b1.bin", chc_select, [8])
        ...
        #1.2
        W_conv2 = weight_variable("w2.bin", chc_select, [3, 3, 8, 8])
        b_conv2 = bias_variable("b2.bin", chc_select, [8])
        ...
        # 2.1
        W_conv5 = weight_variable("w5.bin", chc_select, [3, 3, 8, 16])
        b_conv5 = bias_variable("b5.bin", chc_select, [16])

        h_conv5 = tf.nn.relu(conv2d(h_pool2, W_conv5) + b_conv5)

        # 2.2
        W_conv6 = weight_variable("w6.bin", chc_select, [3, 3, 16, 16])
        b_conv6 = bias_variable("b6.bin", chc_select, [16])

        h_conv6 = tf.nn.relu(conv2d(h_conv5, W_conv6) + b_conv6)
        h_pool6 = max_pool_2x2(h_conv6)

        # 3.1
        W_conv7 = weight_variable("w7.bin", chc_select, [3, 3, 16, 32])
        b_conv7 = bias_variable("b7.bin", chc_select, [32])

        h_conv7 = tf.nn.relu(conv2d(h_pool6, W_conv7) + b_conv7)


        # 3.2
        W_conv8 = weight_variable("w9.bin", chc_select, [3, 3, 32, 32])
        b_conv8 = bias_variable("b9.bin", chc_select, [32])

        h_conv8 = tf.nn.relu(conv2d(h_conv7, W_conv8) + b_conv8)

        # 3.3
        W_conv16 = weight_variable("w17.bin", chc_select, [3, 3, 32, 32])
        b_conv16 = bias_variable("b17.bin", chc_select, [32])

        h_conv16 = tf.nn.relu(conv2d(h_conv8, W_conv16) + b_conv16)

        # 3.4
        W_conv9 = weight_variable("w10.bin", chc_select, [3, 3, 32, 32])
        b_conv9 = bias_variable("b10.bin", chc_select, [32])

        h_conv9 = tf.nn.relu(conv2d(h_conv16, W_conv9) + b_conv9)
        h_pool9 = max_pool_2x2(h_conv9)

        # 4.1
        W_conv10 = weight_variable("w11.bin", chc_select, [3, 3, 32, 64])
        b_conv10 = bias_variable("b11.bin", chc_select, [64])

        h_conv10 = tf.nn.relu(conv2d(h_pool9, W_conv10) + b_conv10)


        # 4.2
        W_conv11 = weight_variable("w12.bin", chc_select, [3, 3, 64, 64])
        b_conv11 = bias_variable("b12.bin", chc_select, [64])

        h_conv11 = tf.nn.relu(conv2d(h_conv10, W_conv11) + b_conv11)

        # 4.3
        W_conv17 = weight_variable("w18.bin", chc_select, [3, 3, 64, 64])
        b_conv17 = bias_variable("b18.bin", chc_select, [64])

        h_conv17= tf.nn.relu(conv2d(h_conv11, W_conv17) + b_conv17)

        # 4.4
        W_conv12 = weight_variable("w13.bin", chc_select, [3, 3, 64, 64])
        b_conv12 = bias_variable("b13.bin", chc_select, [64])

        h_conv12 = tf.nn.relu(conv2d(h_conv17, W_conv12) + b_conv12)
        h_pool12 = max_pool_2x2(h_conv12)

        # 5.1
        W_conv13 = weight_variable("w14.bin", chc_select, [3, 3, 64, 64])
        b_conv13 = bias_variable("b14.bin", chc_select, [64])

        h_conv13 = tf.nn.relu(conv2d(h_pool12, W_conv13) + b_conv13)


        # 5.2
        W_conv14 = weight_variable("w15.bin", chc_select, [3, 3, 64, 64])
        b_conv14 = bias_variable("b15.bin", chc_select, [64])

        h_conv14 = tf.nn.relu(conv2d(h_conv13, W_conv14) + b_conv14)

        # 5.3
        W_conv18 = weight_variable("w19.bin", chc_select, [3, 3, 64, 64])
        b_conv18 = bias_variable("b19.bin", chc_select, [64])

        h_conv18 = tf.nn.relu(conv2d(h_conv14, W_conv18) + b_conv18)

        # 5.4
        W_conv15 = weight_variable("w16.bin", chc_select, [3, 3, 64, 64])
        b_conv15 = bias_variable("b16.bin", chc_select, [64])

        h_conv15 = tf.nn.relu(conv2d(h_conv18, W_conv15) + b_conv15)
        h_pool15 = max_pool_2x2(h_conv15)
        
        #Full connect layer 1
        W_fc1 = weight_variable("w3.bin", chc_select, [4 * 4 * 64, hidden])
        b_fc1 = bias_variable("b3.bin", chc_select, [hidden])

        h_pool15_flat = tf.reshape(h_pool15, [-1, 4 * 4 * 64])
                ...
                for ep in range(epochs):
                    ...
                    for i in range(num//batch_size):
                        ...
                        if step == stepnum and chc_select == hc:
                            ...
                            w1, b1, w2, b2, w5, b5, w6, b6, w7, b7, w9, b9, w17, b17, w10, b10, w11, b11, w12, b12, w18, b18, w13, b13, w14, b14, w15, b15, w19, b19, w16, b16, w3, b3, w8, b8, w4, b4 = sess.run([W_conv1, b_conv1, W_conv2, b_conv2, W_conv5, b_conv5, W_conv6, b_conv6, W_conv7, b_conv7, W_conv8, b_conv8, W_conv16, b_conv16, W_conv9, b_conv9, W_conv10, b_conv10, W_conv11, b_conv11, W_conv17, b_conv17, W_conv12, b_conv12, W_conv13, b_conv13, W_conv14, b_conv14, W_conv18, b_conv18, W_conv15, b_conv15, W_fc1, b_fc1, W_fc3, b_fc3, W_fc2, b_fc2], feed_dict = feed_dict)
                            w1.tofile("w1.bin")
                            b1.tofile("b1.bin")
                            w2.tofile("w2.bin")
                            b2.tofile("b2.bin")
...
#                        print 'label:\n', ll
                ...
                print 'before', l_ind_dis
                print 'set', set(l_ind_dis)
                for i in set(l_ind_dis):
                    d[i] = l_ind_dis.count(i)
                    print '这次是', d[i], '来自', l_ind_dis.count(i)
