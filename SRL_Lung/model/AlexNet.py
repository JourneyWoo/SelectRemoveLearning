...
def cnn_model():
    ...
    for chc in range(3):
        ...
        #First layer
        W_conv1 = weight_variable("w1.bin", chc_select, [11, 11, 1, 4])
        ...
        #Second layer
        W_conv2 = weight_variable("w2.bin", chc_select, [5, 5, 4, 32])
        b_conv2 = bias_variable("b2.bin", chc_select, [32])
        ...
        # Third layer
        W_conv5 = weight_variable("w5.bin", chc_select, [3, 3, 32, 32])
        b_conv5 = bias_variable("b5.bin", chc_select, [32])

        h_conv5 = tf.nn.relu(conv2d(h_pool2, W_conv5) + b_conv5)

        # Fourth layer
        W_conv6 = weight_variable("w6.bin", chc_select, [3, 3, 32, 48])
        b_conv6 = bias_variable("b6.bin", chc_select, [48])

        h_conv6 = tf.nn.relu(conv2d(h_conv5, W_conv6) + b_conv6)

        # Fifth layer
        W_conv7 = weight_variable("w7.bin", chc_select, [3, 3, 48, 32])
        ...
        W_fc1 = weight_variable("w3.bin", chc_select, [16 * 16 * 32, hidden])
        b_fc1 = bias_variable("b3.bin", chc_select, [hidden])

        h_pool7_flat = tf.reshape(h_pool7, [-1, 16 * 16 * 32])
                ...
                for ep in range(epochs):
                    ...
                    for i in range(num//batch_size):
                        ...
                        if step == stepnum and chc_select == hc:
                            ...
                            w1, b1, w2, b2, w5, b5, w6, b6, w7, b7, w3, b3, w8, b8, w4, b4 = sess.run([W_conv1, b_conv1, W_conv2, b_conv2, W_conv5, b_conv5, W_conv6, b_conv6, W_conv7, b_conv7, W_fc1, b_fc1, W_fc3, b_fc3, W_fc2, b_fc2], feed_dict = feed_dict)
                            w1.tofile("w1.bin")
                            b1.tofile("b1.bin")
                            w2.tofile("w2.bin")
                            b2.tofile("b2.bin")
